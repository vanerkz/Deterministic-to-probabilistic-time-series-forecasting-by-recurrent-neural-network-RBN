...
def MAE(pred, true):
    return np.mean(np.abs(pred-true)),np.std(np.mean(np.abs(pred-true),axis=1))
...
def autoplot(pred,true,items):
    fig = plt.figure(figsize=(8, 6))
    
    for plot_id, i in enumerate(items):
        subplots = [321, 322, 323, 324]
        ax1 = fig.add_subplot(subplots[plot_id])
        s=pred[i]-true[i]
        print("index"+str(i)+" mean:" +str(np.mean(s)))
        sm.graphics.tsa.plot_acf(s,ax=ax1)
...
def commonmetric(pred, true):
    ...
    #corr1 = CORR(pred, true)
    corr = Corr(pred, true)
    #dtw_dis = dtwmetrics(bscaled,yscaled)
    dtw_dis=0
    """visualize(pred.flatten(),true.flatten())
    visualize(bscaled.flatten(),yscaled.flatten())"""
    shape = pred.shape
    return mae,mse,rmse,mape,mspe,dtw_dis,corr,shape
